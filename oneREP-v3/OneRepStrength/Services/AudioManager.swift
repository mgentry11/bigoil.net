...
class AudioManager: NSObject, ObservableObject, AVAudioPlayerDelegate {
    ...
    private func configureAudioSession() {
        do {
            // allows background audio and mixing with other apps (if desired, or use .playback for strictly app audio)
    ...
    func speak(_ text: String, audioFile: String? = nil) {
        ...
        if let audioFile = audioFile, voiceStyle == .commander {
            ...
            if playAudioResource(audioFile) {
                ...
                print("[AudioManager] FAILED direct lookup. Commander mode active: SUPPRESSING TTS fallback.")
                // User requested strictly no synthetic voices for Commander.
                return
            }
        }
        
        // Only use TTS if NOT in Commander mode (or if no audioFile provided and not Commander)
        speakText(text)
    }
