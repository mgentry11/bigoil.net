...
class AudioManager: NSObject, ObservableObject, AVAudioPlayerDelegate {
    ...
    // MARK: - Logging
    private let log = OSLog(subsystem: "com.onerepstrength.app", category: "AudioManager")
    ...
    override init() {
        super.init()
        os_log("AudioManager: Initializing", log: log, type: .info)
        configureAudioSession()
        setupInterruptionHandling()
    }

    private func configureAudioSession() {
        os_log("AudioManager: Configuring audio session for playback", log: log, type: .info)
        do {
            // Allow app audio to mix with user's music
            // .duckOthers lowers music volume slightly during voice cues
            try AVAudioSession.sharedInstance().setCategory(.playback, mode: .default, options: [.mixWithOthers, .duckOthers])
            try AVAudioSession.sharedInstance().setActive(true)
            os_log("AudioManager: Audio session configured successfully", log: log, type: .info)
        } catch {
            os_log("AudioManager: ❌ Failed to configure audio session: %{public}@", log: log, type: .error, error.localizedDescription)
        }
    }
    ...
    private func setupInterruptionHandling() {
        ...
        // Register for route changes (headphones connect/disconnect)
        NotificationCenter.default.addObserver(
            self,
        ...
        os_log("AudioManager: Interruption handling configured", log: log, type: .info)
    }
    ...
    @objc private func handleInterruption(_ notification: Notification) {
        ...
        switch type {
        case .began:
            os_log("AudioManager: ⚠️ AUDIO INTERRUPTED - stopping playback", log: log, type: .info)
            isAudioInterrupted = true
            stopAudio()
            
        case .ended:
            os_log("AudioManager: ✅ Interruption ended", log: log, type: .info)
    ...
    @objc private func handleRouteChange(_ notification: Notification) {
        ...
        os_log("AudioManager: 🔊 Audio route changed (reason=%{public}d)", log: log, type: .info, Int(reason.rawValue))
        
        // Reconfigure audio session after device changes
    ...
    func speakText(_ text: String) {
        // Defense-in-depth: Commander mode should NEVER use synthetic voice
        if voiceStyle == .commander {
            print("[AudioManager] speakText blocked - Commander mode does not use synthetic voice")
            return
        }
        ...
        utterance.rate = voiceStyle == .commander ? 0.45 : 0.5
    ...
    func playAudioFile(_ url: URL) {
        os_log("AudioManager: Playing audio file: %{public}@", log: log, type: .info, url.lastPathComponent)
        
        // Clear interrupted flag - music from another app sets this but never clears it
        // We want to play our audio over/with music, not be blocked by it
        if isAudioInterrupted {
            os_log("AudioManager: Clearing interrupted flag - playing audio", log: log, type: .info)
            isAudioInterrupted = false
        }
        ...
        do {
            ...
            audioPlayer?.play()
            isPlaying = true
            os_log("AudioManager: ✅ Playback started", log: log, type: .info)
        } catch {
            os_log("AudioManager: ❌ Failed to play audio: %{public}@", log: log, type: .error, error.localizedDescription)
        }
    }

    func speak(_ text: String, audioFile: String? = nil, interruptPrevious: Bool = false) {
        print("[AudioManager] speak called - text: '\(text)', audioFile: \(audioFile ?? "nil"), voiceStyle: \(voiceStyle)")

        // Only stop current audio if explicitly requested (e.g., countdown numbers)
        // This lets encouragement and phase cues finish naturally
        if interruptPrevious {
            stopAudio()
        }

        // Commander mode: ONLY use audio files, NEVER synthetic voice
        if voiceStyle == .commander {
            guard let audioFile = audioFile else {
                print("[AudioManager] Commander mode: No audio file provided, skipping (no synthetic voice)")
                return
            }
            ...
            if playAudioResource(audioFile) {
                ...
                print("[AudioManager] FAILED direct lookup. Commander mode: NO synthetic voice fallback.")
                return
            }
        }

        // Non-commander modes: use TTS
        speakText(text)
    }
    ...
    private func playAudioResource(_ filename: String) -> Bool {
        ...
        // 2. Check "Audio" directory directly
        ...
        // 3. Recursive search in "Audio" folder (for deep structures like ElevenLabs exports)
        // Note: verify if Audio folder exists first
        if let audioBundlePath = Bundle.main.path(forResource: "Audio", ofType: nil) {
            let fileManager = FileManager.default
            if let enumerator = fileManager.enumerator(atPath: audioBundlePath) {
                for case let file as String in enumerator {
                    if file.hasSuffix(filename + ".mp3") || file == filename + ".mp3" {
                        // Found it! Construct full path
                        let fullPath = (audioBundlePath as NSString).appendingPathComponent(file)
                        print("[AudioManager] Found recursively: \(fullPath)")
                        playAudioFile(URL(fileURLWithPath: fullPath))
                        return true
                    }
                }
            }
    ...
    func playCue(_ text: String, audioFile: String, completion: (() -> Void)? = nil) {
        ...
        // Use the actual audio completion callback for Commander mode (uses audio files)
        if voiceStyle == .commander {
            // Set completion to be called when audio finishes playing
            onAudioComplete = completion
        } else {
            // For TTS modes, use estimated delay based on text length
            let estimatedDuration = max(1.0, Double(text.count) * 0.08)
    ...
    func playCountdownNumber(_ number: Int, isRest: Bool = false) {
        // For final countdown (5, 4, 3, 2, 1), ALWAYS interrupt - must be on time
        // For other numbers (30, 20, 15, 10), skip if audio is still playing
        let isFinalCountdown = number <= 5

        if !isFinalCountdown && isPlaying {
            print("[AudioManager] Skipping countdown \(number) - audio still playing")
            return
        }

        // Stop any currently playing audio for countdown
        if isFinalCountdown || isPlaying {
            stopAudio()
        }

        // During exercise: just say the number (fast, no "remaining")
        // During rest: say "X seconds" for longer warnings, just number for final countdown
        let audioKey: String

        if isRest {
            // Rest mode - can use fuller phrases for big numbers
            switch number {
            case 30: audioKey = AudioFiles.rest30Sec
            case 20: audioKey = AudioFiles.time20Sec
            case 15: audioKey = AudioFiles.rest15Sec
            case 10: audioKey = AudioFiles.time10Sec
            case 5, 4, 3, 2, 1: audioKey = "count_\(number)" // Just number for final countdown
            default: audioKey = "count_\(number)"
            }
        } else {
            // Exercise mode - just say the number, quick and clean
            audioKey = "count_\(number)"
        }

        speak("\(number)", audioFile: audioKey, interruptPrevious: isFinalCountdown)
    }
}
...
extension AudioManager {
    ...
    func playRestStarting() {
        // Skip if something is still playing (like encouragement)
        guard !isPlaying else {
            print("[AudioManager] Skipping rest starting - audio still playing")
            return
        }
    ...
    func playRestGetReady() {
        // This is important - interrupt if needed for the 20 second warning
        speak("Get ready for the next exercise", audioFile: AudioFiles.restGetReady, interruptPrevious: true)
    }
    ...
    func playRestComplete() {
        // This is important - must play when rest ends
        speak("Rest complete", audioFile: AudioFiles.restComplete, interruptPrevious: true)
    }
