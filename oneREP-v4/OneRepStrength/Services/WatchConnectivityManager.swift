...
class WatchConnectivityManager: NSObject, ObservableObject {
    ...
    func sendWorkoutState(_ state: WatchWorkoutState) {
        ...
        do {
            ...
            // Try real-time message if reachable
            if session.isReachable {
                session.sendMessage(message, replyHandler: nil) { [weak self] error in
                    // If message fails, fall back to application context
                    print("Message failed, using context: \(error.localizedDescription)")
                    self?.updateContext(with: state)
                }
            } else {
                // Fall back to application context when not reachable
                updateContext(with: state)
            }
        } catch {
