...
class ThemeManager: ObservableObject {
    ...
    // Glass intensity: 0.0 = no glass, 1.0 = maximum glass effect
    ...
    init() {
        ...
        // Load glass intensity (default to 0.7 for nice glass look)
        self.glassIntensity = UserDefaults.standard.object(forKey: "glassIntensity") as? Double ?? 0.7
    }

    // Glass effect multipliers based on intensity - MUCH more dramatic
    var glassBlur: Double { glassIntensity }
    var glassBorderOpacity: Double { 0.2 + (glassIntensity * 0.8) }  // Up to 1.0 at max
    var glassHighlightOpacity: Double { glassIntensity * 0.5 }       // Up to 0.5 at max
    var glassShadowOpacity: Double { glassIntensity * 0.6 }          // Up to 0.6 at max
    var glassTintOpacity: Double { glassIntensity * 0.35 }           // Up to 0.35 at max
...
struct GlassBackground: View {
    ...
    var body: some View {
        let intensity = themeManager.glassIntensity

        ZStack {
            // Dark base for contrast
            RoundedRectangle(cornerRadius: cornerRadius)
                .fill(Color(hex: "1A1A1F").opacity(0.6 + intensity * 0.3))

            // Thick frosted glass material
            RoundedRectangle(cornerRadius: cornerRadius)
                .fill(.regularMaterial)
                .opacity(intensity * 0.8)

            // Strong color tint overlay
            RoundedRectangle(cornerRadius: cornerRadius)
                .fill(
                    LinearGradient(
                        colors: [
                            tintColor.opacity(isActive ? intensity * 0.4 : intensity * 0.2),
                            tintColor.opacity(isActive ? intensity * 0.15 : intensity * 0.05),
                            Color.clear
                        ],
                        startPoint: .topLeading,
                        endPoint: .bottomTrailing
            ...
            // Bright highlight reflection at top
            RoundedRectangle(cornerRadius: cornerRadius)
                .fill(
                    LinearGradient(
                        colors: [
                            Color.white.opacity(themeManager.glassHighlightOpacity * (isActive ? 2.0 : 1.5)),
                            Color.white.opacity(themeManager.glassHighlightOpacity * 0.3),
                            Color.clear
                        ],
                        startPoint: .top,
                        endPoint: .bottom
                ...
                .stroke(
                    LinearGradient(
                        colors: [
                            ...
                            tintColor.opacity(intensity * 0.4),
            ...
            RoundedRectangle(cornerRadius: cornerRadius)
                .stroke(
                    LinearGradient(
                        colors: [
                            (isActive ? tintColor : Color.white).opacity(themeManager.glassBorderOpacity),
                            Color.white.opacity(0.15),
                            tintColor.opacity(isActive ? 0.5 : 0.2)
                        ],
                        startPoint: .topLeading,
                        endPoint: .bottomTrailing
                    ),
