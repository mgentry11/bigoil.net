...
struct WorkoutListView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Header
            HStack {
                Button(action: { workoutManager.showingProfile = true }) {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 8) {
                        ForEach(ProfileManager.shared.profiles) { profile in
                            ProfileButton(
                                title: profile.name.prefix(1).uppercased() + (profile.name.count > 1 ? String(profile.name.suffix(from: profile.name.index(after: profile.name.startIndex)).prefix(1)) : ""),
                                isSelected: ProfileManager.shared.currentProfile.id == profile.id
                            ) {
                                ProfileManager.shared.switchProfile(to: profile)
                            }
                        }
                    }
                }
    ...
    private func updateSchedule() {
        ...
        let lastWorkoutType = lastLog?.workoutType ?? "A"
...
struct ExerciseIconView: View {
    ...
    var body: some View {
        ...
        if let iconImage = loadIconImage() {
            ...
            // Fallback - gold square with dumbbell
            ZStack {
                RoundedRectangle(cornerRadius: 8)
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    HStack(spacing: 6) {
                        ...
                        if scheduler.nextWorkoutDate != nil {
                            Text("• Workout \(scheduler.nextWorkoutType)")
                                .font(.caption)
                                .foregroundColor(ThemeManager.shared.primary)
                        }
                    }
...
struct RescheduleWorkoutSheet: View {
    ...
    @Environment(\.dismiss) var dismiss
    @State private var selectedWorkoutType: String = "A"
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack(spacing: 12) {
                        WorkoutTypeButton(title: "Workout A", isSelected: selectedWorkoutType == "A") {
                            selectedWorkoutType = "A"
                        }
                        WorkoutTypeButton(title: "Workout B", isSelected: selectedWorkoutType == "B") {
                            selectedWorkoutType = "B"
                        }
                    }
                }
                .padding(.horizontal)
                ...
                // Save Button
                Button(action: {
                    scheduler.setNextWorkoutDate(selectedDate, workoutType: selectedWorkoutType, profile: profile)
                    dismiss()
                }) {
                    HStack {
                        Image(systemName: "checkmark.circle.fill")
        ...
        .preferredColorScheme(.light)
        .onAppear {
            selectedWorkoutType = scheduler.nextWorkoutType
