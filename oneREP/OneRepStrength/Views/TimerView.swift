    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if let exercise = workoutManager.currentExercise {
                ...
                lastAnnouncedPhase = .prep
                audioManager.speak(exercise.name, audioFile: exercise.audioFileName)
                
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
                    audioManager.playCue("Get ready", audioFile: "phase_get_ready") {
        ...
        .onChange(of: workoutManager.currentPhase) { oldPhase, newPhase in
            ...
            if newPhase == .eccentric || newPhase == .concentric || newPhase == .finalEccentric {
                ...
                audioManager.playCue(cueText, audioFile: cueFile) {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                        audioManager.speak("Great work!", audioFile: "enc_great_work")
                        isWaitingForAudio = false
                    }
                }
            }
        }
        .onChange(of: workoutManager.timeRemaining) { oldTime, newTime in
            guard workoutManager.isTimerRunning else { return }
            guard newTime != lastAnnouncedTime else { return }
            guard !audioManager.isCuePlaying else { return }
            guard !isWaitingForAudio else { return }

            let countBy5s: Set<Int> = [40, 35, 30, 25, 20, 15, 10]
            let finalCountdown: Set<Int> = [5, 4, 3, 2, 1]

            if countBy5s.contains(newTime) {
                lastAnnouncedTime = newTime
                audioManager.playCountdownNumber(newTime)
                
                if newTime == 10 {
                    UIImpactFeedbackGenerator(style: .medium).impactOccurred()
                }
            } else if finalCountdown.contains(newTime) {
                lastAnnouncedTime = newTime
                UIImpactFeedbackGenerator(style: .heavy).impactOccurred()
                
                let delay = Double(5 - newTime) * 0.15
